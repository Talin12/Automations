...
class CompressImage(models.Model):
    ...
    compressed_img = models.ImageField(upload_to='compressed_images/', blank=True, null=True)
    compressed_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Image {self.id} by {self.user.username} with quality {self.quality}"
